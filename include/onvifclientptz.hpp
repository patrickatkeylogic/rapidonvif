...
public:
	...
	// Preset functions
	int SetPreset(_tptz__SetPresetResponse& response, std::string token, std::string name, std::string presetToken = "");
	int GetPresets(_tptz__GetPresetsResponse& response, std::string token);
	int GotoPreset(_tptz__GotoPresetResponse& response, std::string token, std::string presetToken);
...
inline int OnvifClientPTZ::SetPreset(_tptz__SetPresetResponse& response, std::string token, std::string name, std::string presetToken)
{
	string strUrl;
	string strUser;
	string strPass;
	...
	setPreset.ProfileToken = token;
	setPreset.PresetName = new std::string(name);

	if (!presetToken.empty())
	{
		setPreset.PresetToken = new std::string(presetToken);
	}


	int res = ptzProxy.SetPreset(&setPreset, &response);
	return res;
}

inline int OnvifClientPTZ::GetPresets(_tptz__GetPresetsResponse& response, std::string token)
{
	string strUrl;
	string strUser;
	string strPass;
	...
	getPresets.ProfileToken = token;

	int res = ptzProxy.GetPresets(&getPresets, &response);
	return res;
}

inline int OnvifClientPTZ::GotoPreset(_tptz__GotoPresetResponse& response, std::string token, std::string presetToken)
{
	string strUrl;
	string strUser;
	string strPass;
	...
	int res = ptzProxy.GotoPreset(&preset, &response);
	return res;
}
